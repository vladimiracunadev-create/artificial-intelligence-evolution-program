...
> [← Clase anterior](../../../classes/part-09-ai-agent-engineering/116-permisos-sandbox-y-minimo-privilegio/README.md) · [Índice de la parte](../README.md) · [Clase siguiente →](../../../classes/part-09-ai-agent-engineering/118-presupuestos-de-pasos-tokens-costo-y-tiempo/README.md)
...
equivocan, y algunas de sus acciones no se pueden deshacer. La matriz de permisos (116)
produce tres decisiones y una de ellas — `ask` — es exactamente esta clase: detener el
bucle, preservar el estado (115), presentar la acción a un humano y reanudar con su
...
- **HITL (human-in-the-loop):** diseño donde ciertos pasos del sistema requieren
  ...
  (sin intervención).
- **Punto de aprobación:** hito bloqueante (112) asociado a una acción o clase de
...
- **Interrupt:** el runtime suspende el bucle en un punto consistente y persiste un
  checkpoint (115) con la acción propuesta y su contexto de justificación.
...
bucle:
    ...
    caso politica(accion):                        # matriz de la clase 116
...
3. **El impacto:** clase de efecto, reversibilidad, alcance ("se enviará a 1.200
   clientes"), y el resultado del `dry_run` cuando exista (113).
...
- **Antídotos a la fatiga de aprobación:** lotes ("aprueba el plan completo de 6
  pasos"), umbrales que auto-aprueban lo trivial, presupuestos pre-aprobados (118), y
...
parecía correcta y no lo era" — insumo directo para las evaluaciones de la clase 119
...
que debería destilarse a memoria semántica (115) y quizá a la política (116). El
...
sequenceDiagram
    ...
    participant P as Política (116)
    participant C as Checkpoint (115)
...
3. **"Rechazar mata la tarea."** El rechazo con motivo es una observación que entra al
   contexto: el agente replantea (112) o escala. Diseñar solo el camino feliz convierte
...
5. **"Interrupt es pausar el proceso en memoria."** Si el veredicto tarda horas, el
   ...
   ingeniería de estado de la clase 115.
...
hasta veredicto, correlación entre lo aprobado y los incidentes (119). Los rechazos
etiquetados alimentan la mejora de la matriz (116) y de los prompts del agente.
...
[118 — Presupuestos de pasos, tokens, costo y tiempo](../../part-09-ai-agent-engineering/118-presupuestos-de-pasos-tokens-costo-y-tiempo/README.md)
