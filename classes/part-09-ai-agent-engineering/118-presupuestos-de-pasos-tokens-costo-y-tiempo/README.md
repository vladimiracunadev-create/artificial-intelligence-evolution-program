...
> [← Clase anterior](../../../classes/part-09-ai-agent-engineering/117-human-in-the-loop-y-aprobaciones/README.md) · [Índice de la parte](../README.md) · [Clase siguiente →](../../../classes/part-09-ai-agent-engineering/119-evaluacion-y-depuracion-de-agentes/README.md)
...
iteración del bucle (111) vuelve a pagar el contexto acumulado y puede añadir pasos que
nadie previó. El presupuesto es al costo lo que la matriz de permisos (116) al riesgo:
...
la parada del plan (112), los checkpoints (115) y la evaluación (119) necesitan para
ser operativos, y es requisito directo del proyecto integrador (120).
...
- **Tokens:** entrada + salida de cada llamada al modelo. Es la moneda del costo
  económico y del contexto (115). Ojo a la asimetría: el contexto se **re-paga** en
...
La consecuencia de diseño: sin compactación (115), duplicar los pasos casi
...
3. **Actuar (al agotarse):** parada limpia con estado parcial (112), checkpoint (115)
...
Cada tool lleva su timeout; los reintentos (113) consumen presupuesto del mismo pozo, y
...
flowchart TD
    ...
    S --> R["Registro para evaluación (119):\ncosto real vs estimado"]
```
...
prod estricto) y revisar mensualmente costo estimado vs real por tipo de tarea (119).
...
[117 — Human-in-the-loop y aprobaciones](../../part-09-ai-agent-engineering/117-human-in-the-loop-y-aprobaciones/README.md)
