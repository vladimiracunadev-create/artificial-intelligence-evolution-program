...
conecta la evidencia (clase 167) y la gobernanza (clase 166) con la acción bajo presión.
...
sin registrar destruye la cadena de evidencia (clase 167). Regla: contener con acciones reversibles y
...
La severidad guía quién se activa (roles de la clase 166) y los tiempos objetivo (SLA de respuesta).
...
```text
Detección     <- monitoreo de alucinación/fairness/calibración (163-165) y red team (159)
Causa raíz    <- inyección/tools (160-161), memorización (162), datos/sesgo (163)
Evidencia     <- logs de trazabilidad y model card/datasheet (167)
Prevención    <- el caso se añade al golden set de regresión (158) y a la matriz de riesgo (166)
```
...
1. **Detección/análisis**: una alerta de red detecta envíos a `externo@atacante.example`; se confirma
   que un correo contenía una instrucción incrustada (inyección indirecta, clase 160). Alcance: 3
...
3. **Erradicación**: causa raíz = la tool aceptaba destinatarios arbitrarios (confused deputy,
   clase 161) y no había separación de contenido no confiable. Parche: allowlist de destinatarios +
   marcado de contenido no confiable + aprobación humana para destinos nuevos.
4. **Recuperación**: antes de reactivar, correr la suite de regresión (clase 158) incluyendo el caso
...
5. **Post-incidente**: análisis de causa raíz documentado; el payload se convierte en **ítem
   permanente del golden set** con severidad alta; se actualiza la matriz de riesgo (clase 166) y la
...
| Detección | ver el daño pronto | depender solo de reportes | monitoreo de métricas de las clases 163-165 |
...
métricas de las clases 163-165, definir severidades y SLA con roles (clase 166), ensayar el plan
(simulacros), integrar la cadena de evidencia (clase 167) y las obligaciones de reporte legal, y
cerrar cada incidente convirtiéndolo en regresión (clase 158) y en un riesgo reevaluado. Este
