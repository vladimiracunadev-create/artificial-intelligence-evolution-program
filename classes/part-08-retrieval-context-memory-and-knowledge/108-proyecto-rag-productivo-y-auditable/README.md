...
> [← Clase anterior](../../../classes/part-08-retrieval-context-memory-and-knowledge/107-evaluacion-de-fidelidad-cobertura-y-atribucion/README.md) · [Índice de la parte](../README.md) · [Clase siguiente →](../../../classes/part-09-ai-agent-engineering/109-de-modelo-y-automatizacion-a-agente/README.md)
...
fusión y re-ranking (100-101), generación con citas (102), transformación de consultas
(103), memoria (105), eficiencia (106) y evaluación (107)— bajo dos requisitos que las
...
El conjunto de evaluación (107) se ejecuta en CI: cada cambio —de prompt, de modelo, de
...
flowchart TD
    ...
    subgraph QRY[Camino de consulta]
        Q[consulta + identidad] --> TR["transformación (103)"]
        TR --> HB["híbrida + RRF (100)<br/>pre-filtro ACL"]
        HB --> RR["re-rank + umbral (101)"]
        RR --> PR["prompt con citas (102)<br/>compresión (106)"]
        PR --> LLM[LLM] --> VA["verificación de<br/>atribución (107)"]
        VA --> U[respuesta con citas]
    end
    IV & IL --> HB
    QRY -.->|traza completa por query_id| OBS[(observabilidad)]
    EV["eval set en CI (107)"] -.->|puerta de despliegue| QRY
```
...
[109 — De modelo y automatización a agente](../../part-09-ai-agent-engineering/109-de-modelo-y-automatizacion-a-agente/README.md)
