...
el ciclo está roto en ese punto: es el problema de **linaje** (lineage) que la clase 146
trata en detalle.
...
AgentOps (clase 153) versiona prompts y herramientas igual que MLOps versiona datasets y
...
1. **¿Ciclo de datos?** El esquema valida, pero la media de la feature `visitas_30d`
   ...
   app que cambió su tracking el viernes. → deriva de datos (clase 151).
...
4. **«Versionar el código basta para reproducir.»** Sin el dataset exacto, la semilla y
   el entorno, el mismo commit produce otro modelo (clase 146).
