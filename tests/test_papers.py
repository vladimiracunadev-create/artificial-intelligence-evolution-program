...
class CatalogTests(unittest.TestCase):
    def test_catalog_is_valid_json_with_148_papers(self):
        data = load_papers()
        self.assertEqual(len(data["papers"]), 148)
        esperado = []
        for bloque in data["rutas"]:
            esperado.extend(data[bloque])
        self.assertEqual(esperado, [item["id"] for item in data["papers"]])
        self.assertEqual(len(data["rutas"]), 17)
...
class ContractTests(unittest.TestCase):
    def test_repository_contract(self):
        ...
        self.assertEqual(result["papers"], 148)
        self.assertEqual(result["notebooks"], 156)
    ...
    def test_manifest_hashes_are_current(self):
        ...
        self.assertEqual(manifest["papers"], 148)
        self.assertEqual(manifest["notebooks"], 156)
